...
(function($) {
    ...
    $(document).ready(function () {
        /* Initialize pixPolish plugin */
        $('#pixPolish').pixPolish({
            baseURL: "./", // The url of the main directory. For example; "http://www.mysite.com/pixPolish-js/"
            ...
            watermarkText: 'https://pixPolish.website/', // The watermark text
            ...
            customFunctions: function(selector, canvas, lazyLoadInstance) {
                ...
                selector.find('.template-grid').on('click','.template-favorite button.star',function(){
                    ...
                    toastr.success("To make 'saving functions' work, you should have a database on your server and integrate it to pixPolish using a server-side language. See Documentation -> Integration.", "Info");
                    // toastr.error("Error!", "Lorem ipsum dolor");
                });

                /* Frame - Add to Favorite */
                selector.find('.pixPolish-frames-grid').on('click','.frame-favorite button.star',function(){
                    var button = $(this);
                ...
                selector.find('.pixPolish-grid').on('click','.element-favorite button.star',function(){
                    var button = $(this);
                ...
                selector.find('.pixPolish-template-list').on('click','.pixPolish-template-delete',function(){
                ...
                selector.find('#pixPolish-library-upload-img').on('change', function (e) {
                ...
                selector.find('.media-library-grid').on('click','.pixPolish-library-delete',function(){
                ...
                selector.find('#pixPolish-svg-library-upload-img').on('change', function (e) {
                    var file_data = this.files[0];
                ...
                selector.find('.svg-library-grid').on('click','.pixPolish-svg-library-delete',function(){
                    var answer = window.confirm("Are you sure you want to delete the image permanently?");
                    if (answer) {
                        var target = $(this).data('target');
                        $(this).parent().remove();
                ...
                // Save preferences
                selector.find('#pixPolish-preferences-save').on('click', function() {
                    var button = $(this);
                    var settings = {};
                    var keys = [];
                    var values = [];
                    selector.find('#pixPolish-preferences .preference').each(function(index, value) {
            ...
            saveTemplate: function(selector, template) {
                ...
                // var name = selector.find('#pixPolish-json-save-name').val();
            ...
            saveImage: function(selector, imgData) {
                var name = selector.find('#pixPolish-save-img-name').val();
                var quality = parseFloat(selector.find('#pixPolish-save-img-quality').val());
                var format = selector.find('#pixPolish-save-img-format').val();
